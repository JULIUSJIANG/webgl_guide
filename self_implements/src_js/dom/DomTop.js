...
export default class DomTop extends NodeModules.react.Component {
    render() {
        ...
        for (let i = 0; i < IndexGlobal.webgl.listAllRecord.length; i += COLUMN_COUNT) {
            ...
            ;
            let listChildrenContainer = new Array();
            for (let j = 0; j < COLUMN_COUNT; j++) {
                ...
                ;
                listChildrenContainer.push(NodeModules.react.createElement(NodeModules.antd.Button, propsBtn, listRecordIJ.inst.getName()));
            }
            ;
            listChildren.push(NodeModules.react.createElement(DomDefine.TAG_DIV, containerStyle, ...listChildrenContainer));
        }
        ;
